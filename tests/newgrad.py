...
from perftorch.locoprop import ConvergenceError, NewGrad
...
class Net(nn.Sequential):
    def __init__(self, feature_factor: int, itr: int, lr: float):
        super(Net, self).__init__()
        self.add_module("input", nn.Conv2d(1, feature_factor, 3, 2))
        for i in range(6):
            self.add_module(f"stage{i}", NewGrad(ConvBlock(feature_factor, 1 + int(i > 0 and i % 2)), itr, lr))
        self.add_module("classifier", NewGrad(Head(feature_factor), itr, lr))
...
def main():
    ...
    losses = {f"[LR={lr:9.6f}][LocalIterations={itr:1d}][Batch={batch:5d}]":
              ...
              for lr in [3 ** -i for i in range(9)] for itr in range(2) for batch in [4 ** i for i in range(3, 8)]}
